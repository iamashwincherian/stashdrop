...
export function getAllItemsWithMeta() {
  const db = getDb();
  const items = (db.prepare("SELECT * FROM items ORDER BY seed_order ASC").all() as unknown as Row[]).map(rowToItem);
  const bucket: Record<string, string> = {};
  (db.prepare("SELECT id, bucket, url, created_at FROM items").all() as { id: string; bucket: string; url: string | null; created_at: number }[]).forEach((r) => {
    bucket[r.id] = r.url ? relativeBucket(r.created_at) : r.bucket;
  });
  const recentOrder = (db.prepare("SELECT id FROM items ORDER BY created_at DESC").all() as { id: string }[]).map((r) => r.id);
...
export function findByUrl(url: string): StashItem | null {
  const target = canonicalizeUrl(url);
  const rows = getDb().prepare("SELECT * FROM items WHERE url IS NOT NULL").all() as unknown as Row[];
...
export function updateItem(id: string, edits: ItemEdits): StashItem | null {
  ...
  db.prepare("UPDATE items SET cluster = @cluster, tags = @tags, note = @note, description = @description, title = @title, kind = @kind, body = @body WHERE id = @id")
    .run({ ...next, id });

  if (edits.cluster && edits.cluster !== current.cluster) {
    const { x, y } = nextPositionForCluster(edits.cluster);
...
export function getItemsMissingEmbeddings(): { id: string; title: string; description: string; tags: string; note: string; highlights: string; body: string | null }[] {
  return getDb().prepare("SELECT id, title, description, tags, note, highlights, body FROM items WHERE embedding IS NULL").all() as {
    id: string; title: string; description: string; tags: string; note: string; highlights: string; body: string | null;
  }[];
}

export function getSearchableItems(): { id: string; title: string; description: string; tags: string; embedding: string | null }[] {
  return getDb().prepare("SELECT id, title, description, tags, embedding FROM items").all() as {
...
export function nextPositionForCluster(cluster: string): { x: number; y: number } {
  const all = getDb().prepare("SELECT cluster, x, y, w, is_text FROM items").all() as unknown as PlacementRow[];
...
export function insertItem(item: StashItem, bucket: string, createdAt: number) {
  const db = getDb();
  const { n } = db.prepare("SELECT COALESCE(MAX(seed_order), 0) + 1 as n FROM items").get() as { n: number };
  db.prepare(`
    INSERT INTO items (id,kind,cluster,x,y,w,title,domain,kept,bucket,is_text,body,playhead,description,tags,highlights,note,related,context,created_at,seed_order,url,image)
    VALUES (@id,@kind,@cluster,@x,@y,@w,@title,@domain,@kept,@bucket,@is_text,@body,@playhead,@description,@tags,@highlights,@note,@related,@context,@created_at,@seed_order,@url,@image)
  `).run({
    ...
    url: item.url ?? null, image: item.image ?? null,
